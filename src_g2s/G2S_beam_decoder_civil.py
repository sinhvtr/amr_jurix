...
def run_beam_search(sess, model, vocab, civil_words_idx, batch, options):
    # Get civil vocab
    ...
    while steps < options.max_answer_len and len(results) < options.beam_size:
        ...
        for i in xrange(cur_size):
            # print(topk_log_probs[i, :])
            # print(topk_ids[i, :])
            # print([vocab.getWord(topk_ids[i, j]) for j in range(len(topk_ids[i]))])
            
            h = hyps[i]
            cur_state = new_states[i]
            cur_context = context_t[i]
            cur_coverage = coverage_t[i]
            topk_ids_civil_index = [j for j in range(len(topk_ids[i])) if topk_ids[i, j] in civil_words_idx and topk_ids[i, 0] != sent_stop_id]

            if len(topk_ids_civil_index) > 0:  
                # print("Words in civil vocab: ", len(topk_ids_civil_index), [vocab.getWord(topk_ids[i, topk_ids_civil_index[j]]) for j in xrange(len(topk_ids_civil_index))])              
                for j in range(len(topk_ids_civil_index)):
                    # attn_dist_t_civil = [attn_dist_t[i, j] for j in topk_ids_civil_index]
                    cur_tok = topk_ids[i, topk_ids_civil_index[j]]
                    # add anony constraint
                    #if cur_tok in vocab.anony_ids and cur_tok not in batch.amr_anony_ids:
                    #    continue
                    cur_tok_log_prob = topk_log_probs[i, topk_ids_civil_index[j]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, help='The path to the log file.')

    args, unparsed = parser.parse_known_args()

    model_prefix = args.model_prefix
    model_dir = model_prefix.split("/")[0]
    ...
    word_vocab = Vocab(model_dir + "/vocab.updated_vocab", fileformat='txt2')
    ...
    civil_words_idx = [word_vocab.to_index_sequence(w) for w in civil_words]
    print(len(civil_words_idx))
    ...
    with tf.Graph().as_default():
        ...
        for i in range(total_num):
            ...
            if mode in ['greedy', 'multinomial']:
                ...
                log_file.write('Instance {} \n'.format(i))
                log_file.write('Sentence input {} \n'.format(cur_batch.sent_inp))
                log_file.write('Reference {} \n'.format(cur_batch.target_ref))
                hyps = run_beam_search(sess, valid_graph, word_vocab, civil_words_idx, cur_batch, FLAGS)
                reffile.write(' '.join(cur_batch.target_ref[0]).encode('utf-8') + "\n")
                for j in xrange(1):
                    hyp = hyps[j]
                    log_file.write('Hypothesis {} \n'.format(hyp))
                    cur_passage = cur_batch.amr_node[0]
                    cur_sent = hyp.idx_seq_to_string(cur_passage, word_vocab, FLAGS)
                    log_file.write(str(cur_sent))
                    log_file.write('\n')
                    cur_sent = cur_sent.encode('utf-8')
                    cur_sent = cur_sent[:len(cur_sent)-4]
                    outfile.write(cur_sent + "\n")
                outfile.flush()
            # exit()
        if mode.endswith('evaluate'):
            ref_outfile.close()
