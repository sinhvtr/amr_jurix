...
def main(_):
    ...
    if os.path.exists(best_path + ".index"):
        ...
        word_vocab = Vocab(log_dir + "/vocab" + ".updated_vocab")
        ...
        print('Collecting vocabs on civil set.')
        (allWords_civil, allChars_civil, allEdgelabels_civil) = G2S_data_stream.collect_vocabs(ftset)
        word_vocab.update_vocab(allWords_civil)
        with open("all_words_civil_idx.txt", "w") as f:
            for w in allWords_civil:
                f.write(w.encode('utf-8') + "\t" + str(word_vocab.to_index_sequence(w)) + "\n") 

        word_vocab.dump_to_txt2(log_dir + "/vocab" + ".updated_vocab")
        char_vocab = None
        if FLAGS.with_char:
